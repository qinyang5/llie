                ...
                if opt['model'] in ['sr', 'srgan'] and rank <= 0:  # image restoration validation
                    ...
                    if opt['dist']:
                        ...
                        for val_data in val_loader:
                            ...
                            visuals = model.get_current_visuals()
                            rlt_img = util.tensor2img(visuals['rlt'])  # uint8
                            # rlt2_img = util.tensor2img(visuals['rlt2'])  # uint8
                            gt_img = util.tensor2img(visuals['GT'])  # uint8
                        ...
                        current_psnr =psnr_total_avg
                        if best_psnr<psnr_total_avg:
                            best_psnr=psnr_total_avg
                            bestiter= iter
                        log_s = ' bestPSNR: {:.4e},bestiter:{}'.format(best_psnr,bestiter)
                        logger.info(log_s)
                        log_s = '# Validation # PSNR: {:.4e}:'.format(psnr_total_avg)
...
#     opt = option.parse(args.opt, is_train=True)
#     opt = option.dict_to_nonedict(opt)
#     model=networks.define_G(opt).cuda()
#
#
...
#     import torch
#
#     input = torch.ones(1, 3, 400, 600, dtype=torch.float, requires_grad=False).cuda()
#     input_m= torch.ones(1, 3, 400, 600, dtype=torch.float, requires_grad=False).cuda()
#     # out = model(input)
#     flops, params = profile(model, inputs=[input,input_m])
#     print('input shape:', input.shape)
#     print('parameters:', params / 1e6)
#     print('flops', flops / 1e9)
#     # print('DICM shape', out.shape)
#
#     dummy_input = torch.randn(1, 3, 400, 600, dtype=torch.float).cuda()
#     dummy_input_m = torch.randn(1, 1, 400, 600, dtype=torch.float).cuda()
