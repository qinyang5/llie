...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride=16, in_chans=3, embed_dim=768, norm_layer=None, flatten=True):
        ...
        self.num_patches = self.grid_size[0] * self.grid_size[1]#16*16
    ...
    def forward(self, x):
        B, C, H, W = x.shape#32,3,400,600
        # assert H == self.img_size[0] and W == self.img_size[1], \
        #     f"Input image size ({H}*{W}) doesn't match model ({self.img_size[0]}*{self.img_size[1]})."

        x = self.proj(x) #8,3,128,128#128,192,14,14 #[32,192,20,30]
        if self.flatten:
            x = x.flatten(2).transpose(1, 2)  # BCHW -> BNC 128,196,192
        x = self.norm(x)#[32,925,192]
        return x
...
class Block(nn.Module):
    def __init__(
        ...
        self.mixer = mixer_cls(dim)#实例化Mamba对象
    ...
    def forward(
        ...
        if not self.fused_add_norm:#x
            ...
            if residual is None:#residual一开始为0
        ...
        hidden_states = self.mixer(hidden_states, inference_params=inference_params)#进vimamba模块训练 输出out
...
def create_block(
    ...
    norm_cls = partial(#将参数固定生成新的调用函数
...
class VisionMamba(nn.Module):
    def __init__(self, 
        ...
        # self.upconv1 = nn.Conv2d(3, 3, 3, 1, 1, bias=True)
        self.proj = nn.ConvTranspose2d(192, 3, kernel_size=patch_size, stride=stride)
        # pretrain parameters
        self.num_classes = num_classes
        self.d_model = self.num_features = self.embed_dim = embed_dim  # 192num_features for consistency with other models

        self.patch_embed = PatchEmbed(# BCHW -> BNC
            img_size=img_size, patch_size=patch_size, stride=stride, in_chans=channels, embed_dim=embed_dim)
        num_patches = self.patch_embed.num_patches#196

        if if_cls_token:
            if use_double_cls_token:#x
                self.cls_token_head = nn.Parameter(torch.zeros(1, 1, self.embed_dim))
                self.cls_token_tail = nn.Parameter(torch.zeros(1, 1, self.embed_dim))
                self.num_tokens = 2
            else:
                self.cls_token = nn.Parameter(torch.zeros(1, 1, self.embed_dim))#1,1,192
                # self.num_tokens = 1
            
        if if_abs_pos_embed:
            self.pos_embed = nn.Parameter(torch.zeros(1, num_patches + self.num_tokens, self.embed_dim))#1,197,192
        ...
        # DICM head
        self.norm_f = (nn.LayerNorm if not rms_norm else RMSNorm)(#RMSNorm
        ...
        # original init
        self.patch_embed.apply(segm_init_weights)#初始化权重
        self.head.apply(segm_init_weights)
        if if_abs_pos_embed:
            trunc_normal_(self.pos_embed, std=.02)#截断正态分布初始化
        if if_cls_token:
            if use_double_cls_token:#x
        ...
        # mamba init
        self.apply(#将_init_weights函数应用到模型的每个子模块
            partial(
                _init_weights,
                n_layer=depth,
    ...
    def forward_features(self, x, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False):
        ...
        B, M, _ = x.shape#128,196,192

        if self.if_cls_token:
            if self.use_double_cls_token:#x
                ...
                if self.use_middle_cls_token:
                    cls_token = self.cls_token.expand(B, -1, -1)#1,1,192->128,1,192
                    token_position = M // 2#98
                    # add cls token in the middle把class token加入到序列中间
        ...
        if self.flip_img_sequences_ratio > 0 and (self.flip_img_sequences_ratio - random.random()) > 1e-5:#x
            x = x.flip([1])
            if_flip_img_sequences = True

        # mamba impl
        residual = None
        hidden_states = x#隐藏状态 128，197，192
        if not self.if_bidirectional:
            for layer in self.layers:#把特征图传入模型

                if if_flip_img_sequences and self.if_rope:#x
                ...
                hidden_states, residual = layer(#residual一开始为0
        ...
        if not self.fused_add_norm:#x
            ...
            hidden_states = fused_add_norm_fn(#RMS归一化
                self.drop_path(hidden_states),
                self.norm_f.weight,
                self.norm_f.bias,
                eps=self.norm_f.eps,
    ...
    def forward(self, x, return_features=True, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False):
        ob,oc,oh,ow=x.shape[0],x.shape[1],x.shape[2],x.shape[3]#8,3,128,128
        x = self.forward_features(x, inference_params, if_random_cls_token_position=if_random_cls_token_position, if_random_token_rank=if_random_token_rank)#return class token 128,192
        b,hw,c=x.shape[0],x.shape[1],x.shape[2]#8,36,192
        scale=int(math.sqrt(int((oh*ow)/hw)))
        h=int(oh/scale)
        w=int(ow/scale)
        x=x.reshape(b,c,h,w).contiguous()#[32,192,30,30]
        x=self.proj(x)#[2,3,128,128]
        #x = hidden_states.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        # x=x.reshape(-1,3,64,64).contiguous()
...
def vim_small_patch16_224_bimambav2_final_pool_mean_abs_pos_embed_with_midclstok_div2(pretrained=False, **kwargs):
    model = VisionMamba(
        patch_size=16, embed_dim=192, depth=24, rms_norm=True, residual_in_fp32=True, fused_add_norm=True, final_pool_type='mean', if_abs_pos_embed=False, if_rope=False, if_rope_residual=False, bimamba_type="v2", if_cls_token=False, if_devide_out=True, use_middle_cls_token=False, **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
        checkpoint = torch.hub.load_state_dict_from_url(
            url="to.do",
