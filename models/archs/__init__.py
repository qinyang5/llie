...
# still on developing...
def build_vssm_model(is_pretrain=False):
    model = VSSM(
        patch_size=4,
        in_chans=3,
        # num_classes=1000,
...
def build_mmpretrain_models(cfg="swin_tiny", ckpt=True, only_backbone=False, with_norm=True, **kwargs):
    ...
    CFGS = dict(
        swin_tiny=dict(
            model=Config.fromfile(os.path.join(config_root, "./swin_transformer/swin-tiny_16xb64_in1k.py")).to_dict()[
                'model'],
        ...
        convnext_tiny=dict(
            model=Config.fromfile(os.path.join(config_root, "./convnext/convnext-tiny_32xb128_in1k.py")).to_dict()[
                'model'],
        ...
        # ================================
        swin_small=dict(
            model=Config.fromfile(os.path.join(config_root, "./swin_transformer/swin-small_16xb64_in1k.py")).to_dict()[
                'model'],
        ...
        convnext_small=dict(
            model=Config.fromfile(os.path.join(config_root, "./convnext/convnext-small_32xb128_in1k.py")).to_dict()[
                'model'],
        ...
        # ================================
        swin_base=dict(
            model=Config.fromfile(os.path.join(config_root, "./swin_transformer/swin-base_16xb64_in1k.py")).to_dict()[
                'model'],
        ...
        convnext_base=dict(
            model=Config.fromfile(os.path.join(config_root, "./convnext/convnext-base_32xb128_in1k.py")).to_dict()[
                'model'],
        ...
        replknet_base=dict(
            ...
            model=Config.fromfile(os.path.join(config_root, "./replknet/replknet-31B_32xb64_in1k.py")).to_dict()[
                'model'],
...
def build_heat_models_(cfg="heat_tiny", ckpt=True, only_backbone=False, with_norm=True,
                       CFGS=dict(
                           heat_mini=dict(
                               ...
                               ckpt=os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                                 "../../../ckpts/classification/heat/heat_mini/ckpt_epoch_296.pth"),
                               tag="model_ema",
                           ),
                           heat_tiny=dict(
                               model=dict(
                               ...
                               ckpt=os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                                 "../../../ckpts/classification/heat/heat_tiny/ckpt_epoch_288.pth"),
                               tag="model",
                           ),
                           heat_small=dict(
                               model=dict(
                               ...
                               ckpt=os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                                 "../../../ckpts/classification/vssm/vssmsmall/ema_ckpt_epoch_276.pth"),
                               tag="model_ema",
                               comment="not finish...",
                           ),
                           heat_base=dict(
                               ...
                               ckpt=os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                                 "../../../ckpts/classification/heat/heat_base/ckpt_epoch_288.pth"),
                               tag="model",
                           ),
                       ),
                       **kwargs):
