...
class PatchMerging2D(nn.Module):
    def __init__(self, dim, out_dim=-1, norm_layer=nn.LayerNorm, channel_first=False):
        super().__init__()#channel_first=False
        self.dim = dim
        Linear = Linear2d if channel_first else nn.Linear
        self._patch_merging_pad = self._patch_merging_pad_channel_first if channel_first else self._patch_merging_pad_channel_last#_patch_merging_pad_channel_last
    ...
    def _patch_merging_pad_channel_last(x: torch.Tensor):
        ...
        x0 = x[..., 0::2, 0::2, :]  # ... H/2 W/2 C 偶数行 偶数列
        x1 = x[..., 1::2, 0::2, :]  # ... H/2 W/2 C 奇数行 偶数列
        x2 = x[..., 0::2, 1::2, :]  # ... H/2 W/2 C 偶数行 技术列
        x3 = x[..., 1::2, 1::2, :]  # ... H/2 W/2 C 奇数行 奇数列
...
class mamba_init:
    ...
    def A_log_init(d_state, d_inner, copies=-1, device=None, merge=True):
        ...
        if copies > 0:
            A_log = repeat(A_log, "d n -> r d n", r=copies)#4 192 16
            if merge:
                A_log = A_log.flatten(0, 1)#768 16
    ...
    def D_init(d_inner, copies=-1, device=None, merge=True):
        ...
        if copies > 0:
            D = repeat(D, "n1 -> r n1", r=copies)#4 192
            if merge:
                D = D.flatten(0, 1)#768
...
class SS2D(nn.Module, mamba_init):
    def __init__(
        ...
        **kwargs,
        ...
        if forward_type in ["v0", "v0seq"]:#v0
    ...
    def __initv0__(
        ...
        **kwargs,
        ...
        super().__init__()
        d_inner = int(ssm_ratio * d_model)#192
        ...
        self.x_proj_weight = nn.Parameter(torch.stack([t.weight for t in self.x_proj], dim=0)) # (K, N, inner)4 38 192
        del self.x_proj
        ...
        self.dt_projs_weight = nn.Parameter(torch.stack([t.weight for t in self.dt_projs], dim=0)) # (K, inner, rank)4 192 6
        self.dt_projs_bias = nn.Parameter(torch.stack([t.bias for t in self.dt_projs], dim=0)) # (K, inner)
        del self.dt_projs
            
        # A, D =======================================
        self.A_logs = self.A_log_init(d_state, d_inner, copies=k_group, merge=True) # (K * D, N)(4*192)768,16
        self.Ds = self.D_init(d_inner, copies=k_group, merge=True) # (K * D)  768
    ...
    def forwardv0(self, x: torch.Tensor, SelectiveScan = SelectiveScanMamba, seq=False, force_fp32=True, **kwargs):
        x = self.in_proj(x)#64,56,56,384
        x, z = x.chunk(2, dim=-1) # (b, h, w, d)64,56,56,192
        z = self.act(z)#64,56,56,192
        x = x.permute(0, 3, 1, 2).contiguous()#64,192,56,56
        x = self.conv2d(x) # (b, d, h, w)#64,192,56,56
        x = self.act(x)
        
        def selective_scan(u, delta, A, B, C, D=None, delta_bias=None, delta_softplus=True, nrows=1):
            return SelectiveScan.apply(u, delta, A, B, C, D, delta_bias, delta_softplus, nrows, False)

        B, D, H, W = x.shape#64,192,56,56
        D, N = self.A_logs.shape#768,16
        K, D, R = self.dt_projs_weight.shape#4,192,6
        L = H * W#3136
        #x_hwwh 把hw转置，堆叠原来的输入和转置的输入，宽高合成一个维度，在channel上相加，64，2，192，3136
        x_hwwh = torch.stack([x.view(B, -1, L), torch.transpose(x, dim0=2, dim1=3).contiguous().view(B, -1, L)], dim=1).view(B, 2, -1, L)
        xs = torch.cat([x_hwwh, torch.flip(x_hwwh, dims=[-1])], dim=1) # (b, k, d, l)
        #再翻折里
        x_dbl = torch.einsum("b k d l, k c d -> b k c l", xs, self.x_proj_weight)#64，4，38，3136
        # x_dbl = x_dbl + self.x_proj_bias.view(1, K, -1, 1)
        dts, Bs, Cs = torch.split(x_dbl, [R, N, N], dim=2)#dts:64,4,6,3136,Bs:64,4,16,3136,Cs:64,4,16,3136
        dts = torch.einsum("b k r l, k d r -> b k d l", dts, self.dt_projs_weight)#64,4,192,3136

        xs = xs.view(B, -1, L) # (b, k * d, l) 64,768,3136
        dts = dts.contiguous().view(B, -1, L) # (b, k * d, l) 64,768,3136
        Bs = Bs.contiguous() # (b, k, d_state, l)64,4,16,3136
        Cs = Cs.contiguous() # (b, k, d_state, l)64,4,16,3136
        ...
        if seq:#x
            ...
            out_y = selective_scan(
                ...
                delta_softplus=True,
            ).view(B, K, -1, L)#SSM过程64，4，192，3136
        assert out_y.dtype == torch.float

        inv_y = torch.flip(out_y[:, 2:4], dims=[-1]).view(B, 2, -1, L)#将得到的y再翻折回来，其中一个是转置的，一个不是
        wh_y = torch.transpose(out_y[:, 1].view(B, -1, W, H), dim0=2, dim1=3).contiguous().view(B, -1, L)
        invwh_y = torch.transpose(inv_y[:, 1].view(B, -1, W, H), dim0=2, dim1=3).contiguous().view(B, -1, L)
        y = out_y[:, 0] + inv_y[:, 0] + wh_y + invwh_y# 64，192，3136
        
        y = y.transpose(dim0=1, dim1=2).contiguous() # (B, L, C) 64，3136，192
        y = self.out_norm(y).view(B, H, W, -1)#64 ，56，56，192
...
class VSSM(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        self.channel_first = (norm_layer.lower() in ["bn", "ln2d"])#LN false
        self.num_classes = num_classes
        self.num_layers = len(depths)#[2,2,27,2]
        ...
        norm_layer: nn.Module = _NORMLAYERS.get(norm_layer.lower(), None)#LayerNorm
        ...
        _make_patch_embed = dict(#_make_patch_embed  stem操作
        ...
        # _make_downsample = dict(#PatchMerging2D
        #     v1=PatchMerging2D,
        ...
        for i_layer in range(self.num_layers):#downsample模块初始化
            ...
            self.layers.append(self._make_layer(#VSS初始化
    ...
    def forward(self, x: torch.Tensor):#64,3,224,224
        # x = self.patch_embed(x)#64,56,56,96
